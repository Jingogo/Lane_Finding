...
def find_lane_pixels(binary_warped):
    ...
    for window in range(nwindows):
        ...
        good_left_inds = np.all([nonzerox < win_xleft_high, nonzerox >= win_xleft_low,
                    nonzeroy < win_y_high, nonzeroy >= win_y_low], axis = 0)
        good_left_inds = good_left_inds.nonzero()[0]
        good_right_inds = np.all([nonzerox < win_xright_high, nonzerox >= win_xright_low,
                    nonzeroy < win_y_high, nonzeroy >= win_y_low], axis = 0)
...
def measure_curvature_real(binary_warped):
    ...
    left_fit_cr, right_fit_cr = fit_polynomial_actual(binary_warped, ym_per_pix, xm_per_pix)
    ...
    left_curverad = ((1+ (2*left_fit_cr[0]*y_eval+left_fit_cr[1])**2)**1.5) / (2*abs(left_fit_cr[0]))
    right_curverad = ((1+ (2*right_fit_cr[0]*y_eval+right_fit_cr[1])**2)**1.5) / (2*abs(right_fit_cr[0]))
